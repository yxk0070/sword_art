import swordManGif from "../assets/role/sword_man.gif";
...
export default function Arena({
  ...
  return (
    ...
    >
      ...
      {showKiRight && (
        <div
          key={`ki-r-${currentTick}`}
          className="absolute top-1/2 w-24 h-24 bg-blue-400 rounded-full blur-xl opacity-0 animate-ki-right z-10 pointer-events-none"
        ></div>
      )}
      {showKiLeft && (
        <div
          key={`ki-l-${currentTick}`}
          className="absolute top-1/2 w-24 h-24 bg-red-400 rounded-full blur-xl opacity-0 animate-ki-left z-10 pointer-events-none"
        ></div>
      )}
      ...
      <div className="absolute bottom-4 w-full flex justify-between px-16 items-end z-20">
        ...
        <div className="relative flex flex-col items-center group">
          ...
          {p1MoveName && (
            <div
              key={`p1-move-${currentTick}`}
              className="absolute -top-12 text-blue-300 font-bold text-lg drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] animate-bounce whitespace-nowrap z-30"
            >
              {p1MoveName}
            </div>
          )}
        ...
        <div className="relative flex flex-col items-center group">
          ...
          {p2MoveName && (
            <div
              key={`p2-move-${currentTick}`}
              className="absolute -top-12 text-red-300 font-bold text-lg drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] animate-bounce whitespace-nowrap z-30"
            >
              {p2MoveName}
            </div>
          )}
